...
def split_content(content, length=800):
    ...
    results = []
    for i in range (len(content_text_chunks)):
        content_result = content.copy()
        content_result['content'] = content_text_chunks[i]
        content_result['order'] = i
...
def get_list_of_results(query, number_of_retries=0):
    start_time = time.time()
    resource_ids = []
    search_options = search_options = {'google': True, 'wikipedia': False, 'arxiv': False,
                          'stackoverflow': False, 'reddit': False, 'miscellaneous': False}
    ...
    try:
        ...
        search_queries_str = handle_chat(human_msg = prompt, messages=messages, context = system_message).strip()
        print(search_queries_str)
    ...
    for i in range(len(content_results)):
        ...
        if content_results[i]['content'].startswith('Error:'):
            print('Error')
        print("\n")
    ...
    if len(content_results) == 0 and len(resource_ids) == 0:
        if number_of_retries < 3:
            messages = [{'author': 'system', 'content': "All you are capable of is providing a reworded version of the users input. You are not capable of producing dialogue."},
                        {'author': 'user', 'content': f"Output a reworded version of: {query}. Output nothing else. Reworded version: "}]
    ...
    with ThreadPoolExecutor(max_workers = cpu_count) as executor:
        ...
        for future in futures:
            split_content_results += future.result()
            future.cancel()
        executor.shutdown(wait=False)
    ...
    '''
    print("NUMBER OF RESULTS: " + str(len(split_content_results)))
...
def search(query):
    ...
    for key in searches_by_links:
        ...
        response_list[-1]['chat'] = [{"author": "user", "content": query, "embedding": query_embedding, "summary": query}]
    response_list = sorted(response_list, key=lambda x: x['match'], reverse=True)
    research = json.dumps(response_list)
    timestamp = time.time()
    with open(f'research{timestamp}.json', 'w') as f:
        f.write(research)
    f.close()
    #os.remove(f'research{timestamp}.json')
    file_name = f'research{timestamp}.json'
    return file_name
